...
TEST_CASE ("1: Testing sane::entities: Create and instantiate a YoutubeChannel entity from map.") {
    ...
    REQUIRE( thumbnails["default"].url              == ENTITY_TEST_001_STRING_PREFIX_THUMBNAIL_DEFAULT );
    REQUIRE( thumbnails["high"].url                 == ENTITY_TEST_001_STRING_PREFIX_THUMBNAIL_HIGH    );
    REQUIRE( thumbnails["medium"].url               == ENTITY_TEST_001_STRING_PREFIX_THUMBNAIL_MEDIUM  );
}