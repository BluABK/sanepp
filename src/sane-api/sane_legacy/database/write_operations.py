...
from database.db_download_tile import DBDownloadTile
from database.engine_statements import update_video_statement_full, get_video_by_vidd_stmt, insert_item, \
    get_video_ids_by_video_ids_stmt, update_extra_information_stmt, update_video_stmt, update_channel_from_remote
from database.models import Channel
from database.orm import engine, db_session
...
class UpdateVideosThread(threading.Thread):
    ...
    def run(self):
        ...
        # bulk_items_to_add = []
        lock.acquire()
        self.logger.db_info("Thread {} - Acquired lock".format(self.db_id, len(items_to_update)))
        ...
        if len(items_to_add) > 0:
            self.logger.db_info("Thread {} - inserting {} new videos".format(self.db_id, len(items_to_add)))
            for i in range(0, len(items_to_add), step):
                engine.execute(Video.__table__.insert(), items_to_add[i:i + step])
        if len(items_to_update) > 0:
            self.logger.db_info("Thread {} - updating {} items".format(self.db_id, len(items_to_update)))
            update_list = []
